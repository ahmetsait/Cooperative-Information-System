        ...
        @if(isset($result[0]))
            ...
            <tr>
                @foreach( $result[0] as  $header_key => $value )
                    <th>{{ config('translations.Cooperatives_TR')[$header_key] }}</th>
                @endforeach
            ...
            @foreach(($result) as $row)
                <tr>
                    ...
                    @endforeach
                    <td>
                        {!! Form::open(['route' => ['cooperatives.destroy', $row->id], 'method' => 'delete']) !!}
                        <div class='btn-group'>
                            <a href="{{ route('cooperatives.show', [$row->id]) }}" class='btn btn-default btn-xs'><i
                                    class="glyphicon glyphicon-eye-open"></i></a>
                            <a href="{{ route('cooperatives.edit', [$row->id]) }}" class='btn btn-default btn-xs'><i
